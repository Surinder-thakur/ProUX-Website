...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${mod.title} — AI Design Workshop | ProUX`,
    description: mod.tagline,
    openGraph: {
      title: `${mod.title} | ProUX AI Design Workshop`,
      description: mod.tagline,
    ...
    alternates: {
      canonical: `https://proux.design/workshops/${mod.slug}`,
    },
  };
}

export default async function WorkshopDetailLayout({ params, children }: Props) {
  const { slug } = await params;
  const mod = getBootcampBySlug(slug);
